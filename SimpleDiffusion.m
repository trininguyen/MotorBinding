...
dtmax = 0.002;%0.00305;%0.007; %difference of time between time steps (s)
t = zeros(step_max,1); %time

car_radi = 280; %cargo radius (nm)
eta = 1e-5;%1.0016e-6;%(pN/nm^2)*s
...
numCTT = 200; %number of CTTs in x direction
    ...
    if MTbound == 0
       ...
       if sqrt(x_m(step,2)^2+x_m(step,3)^2)>bindRadiusMT
        ...
        if tailbound == 0 && any(any(distMotorCTT <= bindRadiusCTT))
            ...
            MT_off = k_MT_dir_off * sqrt(x_m(step,2)^2+x_m(step,3)^2);
